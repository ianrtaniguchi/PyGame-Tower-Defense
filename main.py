...
except NameError: #except dos erros possiveis de falta de pasta
    BASE_DIR = Path(".").resolve() # Fallback para quando executado em ambientes onde __file__ não está definido
    ASSETS_DIR = BASE_DIR / "assets"
    IMAGES_DIR = ASSETS_DIR / "images"
    SOUNDS_DIR = ASSETS_DIR / "sounds"
...
def load_image(filename, placeholder_size, placeholder_color): #Função que carrega cada imagem, se falha, cria um placeholder
    ...
    except (pygame.error, FileNotFoundError): # Captura erro da biblioteca nao achar o arquivo e FileNotFoundError também
...
def load_sound(filename): # Tenta carregar um som, se falhar, retorna um objeto vazio do tipo 'none'
    try:
...
class Enemy(pygame.sprite.Sprite): #classe para os inimigos
    ...
    def draw_health_bar(self, surface): # Desenha a barra de vida acima do inimigo.
        if self.health < self.max_health:
            ...
            bg_rect = pygame.Rect(self.rect.centerx - bar_width/2, self.rect.top - 10, bar_width, bar_height) #barra total
            pygame.draw.rect(surface, RED, bg_rect) # desenha a barra
            
            hp_rect = pygame.Rect(self.rect.centerx - bar_width/2, self.rect.top - 10, bar_width * health_ratio, bar_height) #barra de vida atual
...
class Button: # Classe para os botões de construção na UI.
    ...
    def draw(self, surface, money):
        ...
        # Desenha o custo
        draw_text(f"${self.cost}", font_small, WHITE, surface, self.rect.centerx, self.rect.bottom - 10, center=True)
...
def draw_ui(surface, lives, money, wave, total_waves, buttons): # Desenha a UI  na parte inferior.
    ...
    draw_text(f"Onda: {wave} / {total_waves}", font_medium, WHITE, surface, SCREEN_WIDTH - 200, GAME_HEIGHT + 35)
...
def draw_tower_preview(surface, mouse_pos, tower_type): # Desenha o preview da torre no mouse
    ...
    # Desenha a torre
    surface.blit(image, (mouse_pos[0] - image.get_width()//2, mouse_pos[1] - image.get_height()//2))
...
def main(): # Função principal do jogo
    running = True
    game_state = "START_MENU" # Estados possíveis: START_MENU, PLAYING, GAME_OVER, WIN
    ...
    while running:
        ...
        for event in events: # para cada evento na lista de eventos
            ...
            # --- Eventos por Estado ---
            if game_state == "START_MENU": # se estiver na tela inicial
                ...
                if event.type == pygame.USEREVENT and event.tipo == "enemy_leaked": #
                    ...
                    if lives <= 0:
                        ...
                        if background_music: background_music.stop()
                        
                # Clique do mouse
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    # 1. Clicou em um botão da UI?
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                    ...
                    wave_in_progress = False
                    wave_spawn_list = []
                    if background_music: background_music.play(loops=-1)

        #  Lógica de Update (Baseada no Estado)
        
        if game_state == "PLAYING":
            # Atualiza Botões (para hover)
            ...
            if not wave_in_progress and not enemy_group: #garante que só comece nova wave se a anterior acabou
                if wave < total_waves:
                    ...
                    # ganho o jogo
                    game_state = "WIN"
                    if background_music: background_music.stop() # para a musica de fundo (funciona pois  ela é carregada como objeto Sound)
        ...
        # 6.4. Desenho (Renderização a cada clock)
        
        screen.fill(BLACK)
        
        if game_state == "START_MENU": #landing page
            screen.blit(background_image, (0, 0)) #desenha fundo em 0,0
            draw_text("TOWER DEFENSE", font_large, WHITE, screen, SCREEN_WIDTH/2, SCREEN_HEIGHT/2 - 50, center=True) #nome do jogo
            draw_text("Pressione [ESPAÇO] para começar", font_medium, WHITE, screen, SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 20, center=True) #clicar para começar
...
if __name__ == "__main__": # Garante que o código só rode se o arquivo for executado diretamente
    main()